...
Ray * castRays(Camera * camera, int32_t i, int32_t j, int32_t nx, int32_t ny) {
	...
	Vector* d      = add_vv(mul_vv(mul_sv(-1, camera->proj_distance), camera->proj_normal), 
			add_vv(mul_sv(u, camera->u_vec), mul_sv(v, camera->v_vec)));
	Ray * ray      = (Ray *) malloc(sizeof(Ray));
	ray->origin    = camera->view_point;
	ray->direction = mult_sv(normalize(d), d);
}
...
HitRecord * hitPlane(Surface * surface, Ray * ray) {
	...
	float t = dot_product(add_vv(surface->point, mul_sv(-1, ray->origin)), surface->normal) / bot;
	if (t > 0) {
